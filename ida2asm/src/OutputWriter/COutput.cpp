...
std::pair<const char *, size_t> COutput::getContiguousVariablesStructData()
{
    ...
    auto outputDefine = [this](const String& var, int tableIndex) {
        out("# define ", var, " " kTableVarStem, tableIndex, '.', var, Util::kNewLine);
    };
    ...
    for (size_t i = 0; i < m_tableVarRanges.size(); i++) {
        ...
        for (auto item = range.first; item <= range.second; item = item->next()) {
            if (item->type() == OutputItem::kDataItem) {
                ...
                if (!m_symFileParser.exportedType(dataItem->name()).empty())
                    cppDefines.push_back(dataItem->name());
            }
        ...
        for (auto item = range.first; item <= range.second; item = item->next()) {
            ...
            if (!dataItem->name().empty())
                outputDefine(dataItem->name(), tableIndex);
        }

        if (!cppDefines.empty()) {
            out("#else", Util::kNewLine);
            for (const auto& var : cppDefines)
                outputDefine(var, tableIndex);
        }
    ...
    for (size_t i = 0; i < m_tableVarRanges.size(); i++) {
        ...
        for (auto item = range.first; item <= range.second; item = item->next()) {
            ...
            if (dataItem->numElements() > 0) {
                ...
                if (elem->type() == DataItem::kLabel && !m_references.hasReference(elem->text()))
                    out('&');
            }
...
void COutput::outputCodeAndData()
{
    ...
    for (auto it = m_outputItems.begin(); it != m_outputItems.end(); ++it) {
        ...
        case OutputItem::kDataItem:
            ...
            else if (it->getItem<DataItem>()->name().empty())
                m_dataIndex++;
            break;
...
const OutputItem *COutput::outputDataItem(const OutputItem *item, DataItemOutputFormat format, bool checkTableVar /* = false */)
{
    ...
    if (format == kDeclareExtern) {
        ...
        outputDataItemValue(item, isOffset);
        return item;
    }

    auto exportedType = m_symFileParser.exportedType(dataItem->name());
    bool overrideType = format == kDeclare && !exportedType.empty();

    if (overrideType) {
        if (exportedType.endsWith(']') && exportedType.secondLast() == '[') {
            assert(arraySize);
            out(exportedType.withoutLast(), arraySize, ']');
        } else {
            out(exportedType);
        }
    } else if (isOffset) {
    ...
    while (canConsumeItem(item, item->next())) {
        item = item->next();
        m_dataIndex++;
    }

    if (item != m_outputItems.end() && item->next()->type() == OutputItem::kProc)
        out(Util::kNewLine);

    return item;
}

void COutput::outputDataItemValue(const OutputItem *item, bool isOffset)
{
    ...
    do {
        auto dataItem = item->getItem<DataItem>();
        auto numElements = dataItem->numElements();
        const DataItem::Element *element = dataItem->begin();
        ...
        prev = item;
        item = item->next();
    } while (canConsumeItem(prev, item));

    if (getOutputPtr()[-1] == ',')
        setOutputPtr(getOutputPtr() - 1);
}

bool COutput::canConsumeItem(const OutputItem *prev, const OutputItem *item) const
{
return false;
    assert(prev && prev < m_outputItems.end() && prev->type() == OutputItem::kDataItem && item);

    if (item < m_outputItems.end() && item->type() == OutputItem::kDataItem) {
        auto prevDataItem = prev->getItem<DataItem>();
        auto dataItem = item->getItem<DataItem>();
        return dataItem->name().empty() && dataItem->size() == prevDataItem->size();
    }
    ...
    if (instruction->type() == Token::T_MOV) {
        ...
        if (isOffset) {
            ...
            if (address->text() == kSwitchTableName) {
                ...
                out(kIndent, "switch ((word)D0) {", Util::kNewLine);
...
size_t COutput::getDataItemArraySize(const OutputItem *item)
{
    assert(item && item < m_outputItems.end() && item->type() == OutputItem::kDataItem);
    ...
    do {
        auto dataItem = item->getItem<DataItem>();
        const DataItem::Element *element = dataItem->begin();
        assert(dataItem->numElements() > 0);

        if (dataItem->numElements() > 1 || element->type() == DataItem::kString) {
...
bool COutput::isTableVar(const OutputItem *item)
{
