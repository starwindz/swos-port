...
struct Entry
{
    ...
    word width;
    word height;
    constexpr Entry(int16_t x, int16_t y, word width, word height) : x(x), y(y), width(width), height(height) {}
};
...
public:
    constexpr EntryStringTable(word flags, SwosDataPointer<StringTable> stringTable)
        : EntryElement(kStringTable), flags(flags), stringTable(stringTable) {}
};
...
struct StringTableNative
{
    ...
    constexpr StringTableNative(int16_t *indexPtr, int16_t startIndex, int16_t numPointers)
        : nativeIndex(indexPtr), startIndex(startIndex), numPointers(numPointers) {}
...
public:
    constexpr EntryMultilineTextNative(word flags, const MultilineText *multilineText)
        : EntryElementNative(kMultilineTextNative), flags(flags), multilineText(multilineText) {}
};
