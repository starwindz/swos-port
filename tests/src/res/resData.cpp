    ...
    if (len > kArchiveExtensionLen && !pathCompare(&path[len - kArchiveExtensionLen], kArchiveExtension)) {
        ...
        if (context) {
            ...
            context->read = [](SDL_RWops *context, void *buf, size_t size, size_t num) {
                ...
                auto readItems = gzfread(buf, size, num, handle);
            ...
            context->close = [](SDL_RWops *context) {
                auto handle = reinterpret_cast<gzFile>(context->hidden.unknown.data1);
                assertMessage(gzclose(handle) == Z_OK, "Failed to close GZIP file "s +
                    std::to_string(reinterpret_cast<intptr_t>(handle)));
                SDL_FreeRW(context);
                return 0;
            };
            context->write = [](SDL_RWops *, const void *, size_t, size_t) { return size_t(); };
            context->type = 0x45543377;
        }
        return context;
    } else {
        auto f = SDL_RWFromFile(path, "rb");
        return f ? f : openFile(path, "rb");
    }
}

std::pair<std::unique_ptr<char[]>, int> loadResFile(const char *name)
{
